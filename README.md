...
Two buttons in the bottom-right open secondary windows, drawn regions clicked via `cv2.setMouseCallback` rather than a native toolkit's widgets: **Filters (zoom)** pops the convolutional filter mosaic full-size — useful because 32 tiles read poorly at the size the inline column has room for — and works whether or not `--activations` is on, since the model always computes those activations when it has them; only the inline column is gated by the flag. **Reference** opens `docs/asl-alphabet-reference.png` (see `build_reference.py`) to sign against, in a resizable window. Both toggle closed on a second click, and both notice if you close the popup with its own window control instead — `cv2.imshow` would otherwise silently recreate a window the user just closed, which is why `realtime_translator.Popup` polls `WND_PROP_VISIBLE` rather than trusting its own `open` flag alone. Every HighGUI call a popup makes is wrapped defensively for the same reason: on at least one Windows build, closing a popup with its native X raised something other than `cv2.error` and took the whole translator down with it before that was caught here.

The **main window** gets the same treatment: closing it with the native X is checked at the top of every frame, before that frame draws anything — checking any later, after that frame's own `cv2.imshow`, would just see the window that `imshow` itself had already recreated, which is what made closing it look like it closed and immediately reopened rather than exiting.

On Windows, `realtime_translator._dark_titlebar()` themes each window's native title bar to match the black interior via the DWM API (`DWMWA_CAPTION_COLOR` on Windows 11, falling back to `DWMWA_USE_IMMERSIVE_DARK_MODE`'s dark grey on Windows 10) — a stock white title bar clashes with everything below it otherwise. It's a no-op everywhere else; window decoration is the desktop environment's job on Linux, and there's no equivalent single API to reach for across them.
